...
def inference(
    ...
    audio, mel = model.inference(
        ...
        ref_window_seconds=ref_window_seconds,
    )
    ...
    audio_output = (
        model.mel_config.sample_rate,
        (audio.cpu().squeeze(0).numpy() * 32767).astype(np.int16),
    )  # (samplerate, int16 audio) for gr.Audio
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Accordion("参照音声の詳細設定", open=False):
                    ...
                    ref_window_gr = gr.Slider(label="窓長（秒）", minimum=0.5, maximum=5.0, value=2.0, step=0.5)

                    extra_refs_gr = gr.Files(label="追加参照音声（任意）", file_count="multiple", type="filepath")

                language_gr = gr.Dropdown(label="Language", choices=list(model.supported_languages), value="japanese")

                step_gr = gr.Slider(label="Step", minimum=1, maximum=100, value=16, step=1)
                ...
                cfg_rescale_gr = gr.Slider(
                    label="CFG Rescale", minimum=0.0, maximum=1.0, value=0.7, step=0.05, info="0=off。過剰CFGの飽和抑制"
                )

                cfg_t_min_gr = gr.Slider(label="CFG t_min", minimum=0.0, maximum=0.9, value=0.0, step=0.05)
            ...
            with gr.Column():
                ...
                tts_button = gr.Button(
                    "\U0001f3a7 Generate / 合成", elem_id="send-btn", visible=True, variant="primary"
                )
