...
この事前学習のあと、推論側の改善と話者類似性の掘り下げを別途進めました。推論では Sway Sampling と CFG rescale を採用し、euler 16 ステップで dopri5-25 とほぼ同品質を約 10 倍高速に得られるようになりました。一方でゼロショット話者類似性は、構造変更を伴う再学習（TLA-SA / MRTE）を試したものの本記事の pooled style vector ボトルネックを後付け継続学習では解けず不採用となり、対象話者が決まっているなら fine tuning が確実という本記事の結論を裏づける形になりました。詳細は [研究サマリ](research-summary.md) を参照してください。
