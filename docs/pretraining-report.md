...
3. ~~研究フェーズ（アーキテクチャ改善調査 + Phase 1 推論改善）~~ → 完了。[architecture-improvement-research.md](architecture-improvement-research.md) にまとめ、Sway Sampling（低ステップで dopri5 品質・約10倍高速）と CFG rescale を実装・webui 既定に採用。BigVGAN v2（MIT）ボコーダも追加
...
6. **Phase 2（再事前学習）** — logit-normal timestep + EMA でのレシピ改善（未着手）
7. **インスタンスの後始末** — 成果物は全て回収済み。追加検証のため当面維持（2026-07-06 判断、$0.9/h ≈ $22/日）。検証完了後に破棄判断
