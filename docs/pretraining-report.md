...
- 採用モデル: `checkpoints/tsukuyomi_ft200.pt`（= ft epoch 200）。webui で使う場合は `webui.py` のチェックポイントパスをこのファイルに書き換える
...
4. **インスタンスの後始末** — 成果物は全て回収済み。追加検証のため当面維持（2026-07-06 判断、$0.9/h ≈ $22/日）。検証完了後に破棄判断
