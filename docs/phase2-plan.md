...
## 7. Phase 2 実施結果（2026-07-07）— 不採用

R2 を実施し、既存 japanese-378h との 3-way 比較で評価した。**結論: logit-normal + EMA はこの条件（378h・15ep・31M）で品質を有意に改善せず、不採用。**

### 学習（R2）

upstream `checkpoint_0.pt` から moe-speech 378h を 15 epochs 継続学習（`timestep_sampling=logit_normal(0,1)` + EMA(decay 0.9995 / warmup 10)、batch 32、Tier1/2 高速化＝bf16 AMP + TF32 + fused AdamW）。vast 2×RTX5090 で実測 ~2.5h。損失は順調に収束、bf16 で NaN なし、EMA も全 epoch 正常生成。成果物は `checkpoints/vast_run2/`。

### 評価（3-way）

held-out 3話者 × 日本語5文。既存 japanese-378h（cosine+EMA無）vs R2生（logit_normalのみ）vs R2-EMA。指標: CER（faster-whisper large-v3）/ 話者類似性（ECAPA cos）/ mel飽和 / 聴感。スクリプト `temps/phase2_eval/eval_phase2_3way.py`、結果 `results_3way.json`（sway−1.0）・`results_3way_dopri25.json`（dopri25）、集計 `agg_final.py`。

**推論設定バイアスに注意（§6 リスク1 が的中）**: 初回は実運用設定（euler16 / sway−1.0）で比較したが R2 の CER が 0.025 と悪化して見えた。これは logit-normal 学習後は sway−1.0（cosine 一致 warp）が不適合なため。同一 R2-EMA でも sway−1.0 で CER=0.107 に対し dopri25 で CER=0.007（16倍差）。→ 公平比較には推論スケジュール非依存の dopri25 で全モデルを揃える必要があった。
...
| r2_raw（logit_normalのみ） | 0.0027 | 0.6278 | 2.588 | 0.790 |
| r2_ema（logit_normal+EMA） | 0.0027 | 0.6304 | 2.587 | 0.786 |

- **CER**: dopri25 で揃えれば R2 も baseline と同等（logit-normal は発音を悪化させない）
- **話者類似性**: R2 は baseline より一貫して −0.02 劣化（全3話者で baseline > R2）
- **EMA**: r2_raw→r2_ema で +0.003 の微改善だが baseline に届かない
- **聴感 A/B**（dopri25, baseline vs r2_ema, `temps/phase2_eval/ab_listen/`）: ユーザー確認で「差を体感できない」
