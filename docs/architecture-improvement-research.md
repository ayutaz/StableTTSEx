...
作成日: 2026-07-06（最終更新: 2026-07-08、Phase 1–3 実施完了・Phase 3 クローズ）/ 対象: StableTTSEx（upstream v1.1、31.6M パラメータ）/ 関連文書: [研究サマリ](research-summary.md)・[事前学習レポート](pretraining-report.md)・[Phase 2 計画/結果](phase2-plan.md)・[Phase 3 計画/結果](phase3-plan.md)
...
4. 推奨順序（当初計画）: まず**再学習不要の推論改善**（Sway Sampling・interval CFG・複数参照平均）→ 次に**チェックポイント互換のレシピ変更**（logit-normal・EMA）で再事前学習 → 効果を確認してから**非互換の構造変更**（アクセント埋め込み・QK-Norm・参照条件付けの sequence 化）に進む。
5. **実施後の到達点（2026-07-08、Phase 1–3 完了）**: Phase 1（推論のみ）は Sway Sampling と CFG rescale を**採用**（低ステップで dopri5 品質・約10倍高速、§9）。Phase 2（logit-normal + EMA のレシピ再学習）は話者類似性が baseline 比 **−0.020** で**不採用**。Phase 3（構造変更再学習）は TLA-SA（**−0.009**）・MRTE（**−0.007**）とも**不採用でクローズ**。**残る最大の伸びしろは依然ゼロショット話者類似性**で、これは §4 の pooled style vector ボトルネックに起因する。今回の否定的結果が示したのは「**既に pooled style vector で学習し切った重みへ、レシピや後付け構造で継続学習しても類似性は改善しない**」ことであり（§10）、ボトルネック打破にはスクラッチ再学習かデータ拡張という大きめの投資が要る。
...
**Phase 3 での構造的確認（2026-07-08）**: この pooled ボトルネックは机上の推論に留まらず、Phase 3 の2実験で裏づけられた。参照 mel 系列への cross-attention（MRTE）を後付けしても gate はほぼ開かず（cross_gate 0.0006–0.0009）、中間表現を事前学習話者埋め込みに整列させる補助損失（TLA-SA）も下流へ転移せず、いずれも話者類似性を改善しなかった（§10）。既存重みが pooled 表現で局所最適に達しているため後付け機構が使われない、という「後付け継続学習では pooled ボトルネックを緩和できない」ことの直接証拠である。
...
**Phase 1 — 推論のみ・再学習なし（施策 1,2,3,4,15）→ 一部採用**
...
**実績（§9）**: Sway Sampling（sway_coef=−1.0）と CFG rescale（0.7）を**採用**（euler16+sway−1.0 が dopri5-25 とほぼ同品質で約10倍高速）。BigVGAN v2（施策15）も追加。SLG（施策4）は6層デコーダで全条件悪化のため**不採用**。複数参照平均・interval CFG は既定無効で温存。webui 既定は solver=euler / step=16 / sway_coef=−1.0 / cfg_rescale=0.7。

**Phase 2 — レシピのみの再事前学習（施策 5,6 ± 7）→ 不採用**
...
**実績（§10）**: logit-normal + EMA を upstream から15ep 再学習。CER は維持したが話者類似性が baseline 比 **−0.020**（3話者一貫）で、聴感 A/B でも差を体感できず**不採用**。

**Phase 3 — 非互換の構造変更をまとめて1回で（施策 8,9,10）→ TLA-SA・MRTE 実施・両方不採用でクローズ**
...
**実績（§10）**: 主目標をゼロショット話者類似性に絞り、参照条件付けの構造変更2案を実施した — TLA-SA（補助話者整列損失、**−0.009**）と MRTE（参照 mel 系列への cross-attention、**−0.007**）はいずれも baseline を上回れず**不採用でクローズ**。共通の失敗要因は「pooled 済み重みへの後付け継続学習」（§10）。日本語アクセント埋め込み・QK-Norm はこの回では未実施で、今後の方向性へ送った。

**Phase 4 — 研究テーマ（施策 11,13,14）→ 今後の方向性へ統合**
P-Flow 式 prompt 化 or infilling 化は Phase 3 の結果を見てから。パラメータ 31M→50-100M への増量もこの段階で検討（Flamed-TTS-Small 76M の実証あり）。**Phase 3 クローズを受け、pooled ボトルネックを本当に解く投資（MRTE スクラッチ学習・infilling 全面移行・データ拡張・日本語アクセント高度化）を「今後の方向性」として §10 に整理した。**
...
## 10. Phase 2・3 実施結果と総括（2026-07-08 クローズ）

§7 のロードマップに沿って Phase 2（レシピのみの再学習）と Phase 3（非互換の構造変更再学習）を実施した。主目標は §4 のゼロショット話者類似性の改善。評価は **dopri25 で揃えた公平比較**（sway−1.0 は cosine 学習前提で logit-normal に不公平なため揃えない）・held-out 3話者×日本語文の **ECAPA-TDNN spk_cos** を主指標、CER（faster-whisper）を副指標とした（整列教師の SV は評価 ECAPA と別系統に固定してテストへのリークを回避）。結論として **3施策すべてが baseline を上回れず不採用**となった。

| フェーズ | 施策 | 初期値 | baseline spk_cos | 施策 spk_cos | 差 | CER | 結論 |
|---|---|---|---|---|---|---|---|
| Phase 2 | logit-normal + EMA | upstream→15ep | 0.6502 | 0.6278（raw）/ 0.6304（EMA） | **−0.020** | 0.0013→0.0027（維持） | 不採用 |
| Phase 3-1 | TLA-SA（WavLM 教師・補助損失） | upstream→15ep | 0.6502 | 0.6408 | **−0.009** | 0.0013→0.0199（微悪化） | 不採用 |
| Phase 3-2 | MRTE（参照 cross-attention） | japanese-378h→15ep | 0.6502 | 0.6432 | **−0.007** | 0.0013（同等） | 不採用 |

各条件 n=15。設計・数値・原因分析の詳細は [phase2-plan.md §7](phase2-plan.md)（Phase 2）・[phase3-plan.md §11–13](phase3-plan.md)（TLA-SA / MRTE / 総括）、研究アーク全体の俯瞰は [research-summary.md](research-summary.md) を参照。

**Phase 2（logit-normal + EMA）**: 発音（CER）は悪化させないが、話者類似性が3話者一貫して −0.020 劣化。聴感 A/B でもユーザーが差を体感できないと確認。**安価なレシピ改善では品質は向上しない**ことを実証した。

**Phase 3-1（TLA-SA、arXiv:2511.09995）**: デコーダ中間表現を凍結事前学習話者埋め込み（WavLM-base-plus-sv、512次元）に cosine 整列させる補助損失。推論時ヘッド破棄で推論ビット不変・state_dict 不変。tla_sa_loss が ~0 に張り付き（層別 projection head が表現力十分でデコーダ本体を弱くしか使わず整列を達成）下流に転移せず、加えて WavLM の英語 SV が日本語話者へ転移しにくかった。

**Phase 3-2（MRTE、MegaTTS2/CAST-TTS 系）**: 参照 mel を系列のまま cross-attention 参照（query=noisy mel、key/value=参照 mel 系列）。DiTConVBlock に zero-init gate 付きで追加直後はビット一致。**cross_gate が 0.0006–0.0009 までしか開かず**、japanese-378h からの継続学習ではモデルが既に pooled c で十分学習済みのため cross-attn を使うインセンティブが弱かった（生 Parameter zero-init ゲートは adaLN ゲートより立ち上がりも遅い）。

### 共通の失敗要因

3施策に共通する失敗要因は「**既に pooled style vector で学習し切った重みへ、後から機構を足して継続学習した**」こと。既存の到達点が局所最適で、新機構（系列 cross-attention・整列補助損失）を使わない方向へ収束する。**§4 の pooled ボトルネックの緩和には「後付け継続学習」は原理的に向かない**ことを、2回の構造変更実験（TLA-SA・MRTE）で実証した — 価値ある否定的結果である。

### 今後の方向性（別途計画）

pooled ボトルネックを本当に解くには、後付けではなく以下のいずれかの大きめの投資が要る。

1. **MRTE をスクラッチ学習**: upstream checkpoint_0 から MRTE 込みで学習し、pooled 依存を最初から作らせない。実装（`models/` の cross-attention 一式）は既存で流用可、config 変更のみ。
2. **in-context infilling への全面移行**（F5/E2/ZipVoice 型）: 類似性の上限は最も高いが、既存 checkpoint を捨てるリビルドで、378h では暗黙アラインメントが不安定になるリスク。
3. **データ拡張**（378h → 数千時間）: 話者多様性がゼロショット類似性の上限を決めるため、機構よりデータ量が効く可能性。
4. **日本語アクセントの高度化**: 話者類似性ではなく日本語品質側の改善。記号挿入方式は実装済みで、音素ごとの連続高低・アクセント核位置の埋め込み追加が次段。

いずれも既定 False で残置した実装資産（`models/tla_sa.py`・`models/diffusion_transformer.py` の CrossAttention・`estimator.py`/`flow_matching.py`/`model.py` の MRTE 透過経路・`precompute_spk_emb.py`）を再利用できる。現状の config は **use_mrte=False / use_tla_sa=False / timestep_sampling=cosine / EMA無** で、推論経路・n_vocab・チェックポイント形式は既存チェックポイントとビット一致（strict ロード可能）に保たれている。
