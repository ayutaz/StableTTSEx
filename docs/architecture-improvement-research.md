...
**結論**: Phase 1 の確実な成果は **Sway Sampling（低ステップで dopri5 品質・約10倍高速）** と **CFG rescale（飽和抑制の安全な小改善）** の2点。両者とも既存の公開チェックポイントにそのまま効く。複数参照平均・interval CFG・SLG は既定無効のまま、必要時のオプションとして温存。webui の既定を `solver=euler / step=16 / sway_coef=−1.0 / cfg_rescale=0.7` に寄せる価値がある（別途聴感確認の上で）。
